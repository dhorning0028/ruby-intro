# To run this code, be sure your current working directory
# is the same as where this file is located and then run:
# ruby 3-arrays.rb

# Arrays are lists of things
favorite_foods = ["tacos", "pizza", "ice cream"]
puts favorite_foods

# we_have_to_back = [4, 8, 15, 16, 23, 42]
# puts we_have_to_back

# mixed_array = ["tacos", 12, true]
# puts mixed_array

shopping_list = [["coffee", "eggs", "milk"], ["beer", "tacos"]]
puts shopping_list
# array of array - arrays embedded in one another

# # Accessing the array
# puts favorite_foods[0]
# puts favorite_foods[1]
# puts favorite_foods[3]
# puts favorite_foods[-1] #last element in the array

# puts shopping_list[1][1]


# Add to the array
favorite_foods.push("more tacos") #adds to the array
puts favorite_foods

# favorite_foods = favorite_foods + ["more tacos"]
# p favorite_foods

# puts favorite_foods.size
# puts favorite_foods.count
# puts favorite_foods.length


# There are lots of fun things you can do with arrays:
# https://ruby-doc.org/core-2.7.0/Array.html
